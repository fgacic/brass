...
const WHEEL_ZOOM_SENSITIVITY = 0.0078;
const WHEEL_ZOOM_PER_EVENT_MIN = 1 / 1.12;
const WHEEL_ZOOM_PER_EVENT_MAX = 1.12;

function parseConnectionEndpoints(connId) {
  // Sort longest-first so 'stokeOnTrent' is matched before 'stone', etc.
  const knownLocations = Object.keys(LOCATION_POSITIONS).sort(
    (a, b) => b.length - a.length
  );
  for (const loc of knownLocations) {
    if (connId.startsWith(loc + "-")) {
      const rest = connId.slice(loc.length + 1);
      const other = knownLocations.find(
        (l) => rest === l || rest.startsWith(l)
      );
  ...
  return (
    <g>
      {slots.map((slot, idx) => {
        ...
        const spreadDeg =
          total === 1 ? 0 : total === 2 ? 60 : total === 3 ? 55 : 42;
        ...
        if (slot.demandKind === "tripleForeign") {
          ...
          return (
            <g key={`${keyPrefix}-${idx}`} pointerEvents="all" cursor="help">
              <title>
                {`Foreign market: cotton, manufacturer, pottery. Beer for this strip: ${
                  hasBeer ? "available" : "sold"
                }.`}
              </title>
              <circle cx={cx} cy={cy} r={pieR + 3} fill="transparent" />
              <path
                d={pieSectorPath(cx, cy, pieR, -90, 120)}
                fill={c}
                stroke="#1c1917"
                strokeWidth={1.2}
              />
              <path
                d={pieSectorPath(cx, cy, pieR, 30, 120)}
                fill={m}
                stroke="#1c1917"
                strokeWidth={1.2}
              />
              <path
                d={pieSectorPath(cx, cy, pieR, 150, 120)}
                fill={p}
                stroke="#1c1917"
                strokeWidth={1.2}
              />
        ...
        return (
          <g key={`${keyPrefix}-${idx}`} pointerEvents="all" cursor="help">
            <title>{`Foreign market: ${label}. Beer for this strip: ${
              hasBeer ? "available" : "sold"
            }.`}</title>
...
function renderSlotGridEmptyCell({
  keyPrefix,
  cx,
  cy,
  x,
...
export function Board({ gameState, playerId, boardFx = null }) {
  ...
  const selectedConnectionIds = new Set(
    selectedTargets.filter((t) => t.type === "connection").map((t) => t.id)
  );

  const myPlayer = gameState.players.find((p) => p.id === playerId);
  const selectedCardObj =
    myPlayer?.hand?.find((c) => c.id === selectedCard) || null;
  const buildValidLocations = buildValidLocationSet(
    selectedAction,
    selectedCardObj,
    gameState,
    buildIndustry
  ...
  const handleWheel = useCallback(
    (e) => {
      ...
      let zoomFactor = Math.exp(dy * WHEEL_ZOOM_SENSITIVITY);
      zoomFactor = Math.min(
        Math.max(zoomFactor, WHEEL_ZOOM_PER_EVENT_MIN),
  ...
  return (
    <div className="relative h-full w-full overflow-visible">
      <svg
        ...
        style={{
          touchAction: "none",
          cursor: panRef.current.hasDragged ? "grabbing" : "default",
        }}
        onWheel={handleWheel}
      ...
      >
        ...
        {Object.entries(gameState.board.links).map(([connId, link]) => {
          ...
          const linkType =
            meta.canal && meta.rail ? "both" : meta.canal ? "canal" : "rail";
          const style = LINK_STYLE[linkType];

          const ownerColor = isBuilt
            ? PLAYER_COLORS[
                gameState.players.find((p) => p.id === link.ownerId)?.color
              ] || "#888"
            : style.stroke;
          ...
          const isWrongEra =
            (era === "canal" && !meta.canal) || (era === "rail" && !meta.rail);
          ...
          return (
            <g key={connId} opacity={!isBuilt && isWrongEra ? 0.25 : 1}>
              ...
              <m.line
                ...
                y2={toPos.y + ny}
                stroke={
                  isSelected ? "#f59e0b" : isBuilt ? ownerColor : style.stroke
                }
                strokeWidth={isSelected ? 5 : isBuilt ? 4 : style.width}
                strokeDasharray={
                  isSelected ? "none" : isBuilt ? "none" : style.dash
                }
                pointerEvents="none"
                initial={
        ...
        {(() => {
          ...
          const ownerColor = isBuilt
            ? PLAYER_COLORS[
                gameState.players.find((p) => p.id === kwLink.ownerId)?.color
              ] || "#888"
            : style.stroke;
          ...
          return (
            <g
              key="farmBrewery2-trunk-stem"
              opacity={!isBuilt && isWrongEra ? 0.25 : 1}
            >
              {isTargetable && (
                <line
                  x1={fPos.x}
        ...
        {Object.entries(LOCATION_POSITIONS).map(([locId, pos]) => {
          ...
          const merchantData = isMerchant
            ? gameState.board.merchants?.[locId]
            : null;
          const tilesHere = gameState.industryTilesOnBoard.filter(
          ...
          const slotGridGeo = useSlotGrid
            ? computeSlotGridGeometry(pos, slots.length)
            : null;
            ...
            >
                      ...
                      {slotGridGeo.cells.map((cell) => {
                        ...
                        const tile = slot.tileId
                          ? tilesHere.find((t) => t.id === slot.tileId) ||
                            tilesHere.find(
                              (t) => t.slotIndex === cell.slotIndex
                            )
                          : null;
                        const { cx, cy, x, y, size } = cell;
                        if (tile) {
                          const ownerPlayer = gameState.players.find(
                            (p) => p.id === tile.ownerId
                          );
                          const outlineColor = ownerPlayer
                            ? PLAYER_COLORS[ownerPlayer.color]
                            : "#555";
                          const tilePop = boardFx?.tilePopId === tile.id;
                          const tileFlip = tileFlipSet.has(tile.id);
                          const tileMotionInitial =
                            tilePop && !reduceMotion
                              ? { scale: 0.65, opacity: 0.88 }
                              : false;
                          ...
                          return (
                            ...
                            >
                              <m.g
                                layoutId={
                                  tilePop ? "brass-build-pending" : undefined
                                }
                                style={{
                              ...
                              >
                                <rect
                                  ...
                                  fill={
                                    tile.isFlipped
                                      ? "#1a1a1a"
                                      : INDUSTRY_COLORS[tile.industry] || "#555"
                                  }
                                  stroke={outlineColor}
                                  strokeWidth={1.5}
                                />
              ...
              {!useSlotGrid && (
                <>
                  ...
                  />
                  {isMerchant &&
                    merchantDemandSlots.length > 0 &&
                    renderMerchantDemandBadges(
                      pos,
                      baseR,
                      merchantDemandSlots,
                      `merch-${locId}`
                  ...
                  {tilesHere.map((tile, idx) => {
                    const angle =
                      (idx * 2 * Math.PI) / Math.max(tilesHere.length, 1) -
                      Math.PI / 2;
      ...
      >
        {[
          ...
          <div key={label} className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-medium text-amber-100/55">
              {label}
            </span>
          </div>
        ))}
        ...
        <div className="mt-0.5 w-fit max-w-[9.5rem] border-t border-amber-900/25 pt-1">
          <div className="flex items-start gap-1.5">
            ...
            <span className="text-[8px] font-medium leading-snug text-amber-100/48">
              Gold dot under each merchant demand icon: beer for that strip
              (hollow when used). Refill at rail era.
            </span>
          </div>
        </div>
        <p className="w-fit max-w-[9.5rem] border-t border-amber-900/25 pt-1 text-[8px] leading-snug text-amber-100/38">
          Triple C/M/P disc: one demand strip, one beer below it. Map tooltips
          on hover.
        </p>
      </m.div>
        ...
        {INDUSTRY_LEGEND_ORDER.map((industryId, idx) => {
          ...
          return (
            <div
              key={industryId}
              className={
                idx > 0 ? "mt-1 border-t border-amber-900/25 pt-1" : "mt-0.5"
              }
            >
...
function buildValidLocationSet(
  selectedAction,
  cardObj,
  gameState,
  buildIndustry
  ...
  if (!cardObj) {
    if (buildIndustry)
      return locationIdsWithEmptySlotForIndustry(gameState, buildIndustry);
    return null;
  }
